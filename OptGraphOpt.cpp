...
public:
    ...
    const std::list<std::string>& GetAttributes() const { return m_attributes; }
...
private:
    ...
    std::unordered_map<std::string, std::list<Block*> > attribute_map;
public:
    ...
    void AddBlock(Block* block)
    {
        ...
        block->SetName(GetUniqueBlockNameInGraph(block));
        
        //updateAttributeMap(block);
        for (auto i : block->GetAttributes()) {
            if ( attribute_map.find(i) == attribute_map.end()) {
                attribute_map[i] = {};
            }
            attribute_map[i].push_back(block);
        }
    ...
    /*void updateAttributeMap(Block* block) {
        for (auto i : block->GetAttributes()) {
            if ( attribute_map.find(i) == attribute_map.end()) {
                attribute_map[i] = {};
            }
    ...
    const std::list<Block*>& GetBlocksWithAttribute(const std::string& attribute) const
    {
    ...
    std::string GetUniqueBlockNameInGraph(Block* block)
    {
        ...
        std::string potentialName = originalName;
        auto it = name_map.find(originalName);
        if (it != name_map.end()) {
            name_map[originalName] = 1;
        } else {
            //originalName = originalName + std::to_string(name_map[originalName]);
            std::ostringstream s;
            s << originalName << name_map[originalName];
            potentialName = s.str();
            name_map[originalName]++;
        }
...
int main(int argc, char* argv[])
{
    ...
    std::cout << "Total program time: " << timeAddBlocks.count() +  timeFindAttributesByBlock.count() + timeFindBocksByAttribute.count() << "ms.\n";
